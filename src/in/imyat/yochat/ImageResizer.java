...
public class ImageResizer {
	...
	/*
	 * Call this static method to resize an image to a specified width and
	 * height.
	 * 
	 ...
	 */
	public static Bitmap resizeImage(byte[] imageData, int targetWidth,
			int targetHeight) {
		...
		Bitmap reducedBitmap = BitmapFactory.decodeByteArray(imageData, 0,
				imageData.length, options);
		Bitmap resizedBitmap = Bitmap.createScaledBitmap(reducedBitmap,
				targetWidth, targetHeight, false);

		return resizedBitmap;
	}

	public static Bitmap resizeImageMaintainAspectRatio(byte[] imageData,
			int shorterSideTarget) {
	...
	public static Pair<Integer, Integer> getDimensions(byte[] imageData) {
		...
		// Only decode the bounds of the image, not the whole image, to get the
		// dimensions
